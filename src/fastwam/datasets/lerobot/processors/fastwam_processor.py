...
class FastWAMProcessor(BaseProcessor):
    ...
    def preprocess(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Preserve episode_index, frame_index, index for prompt lookup
        if "episode_index" in data:
            sample["episode_index"] = data["episode_index"]
            logger.debug(f"[FastWAMProcessor.preprocess] Preserved episode_index: {sample['episode_index']}")
        if "frame_index" in data:
            sample["frame_index"] = data["frame_index"]
            logger.debug(f"[FastWAMProcessor.preprocess] Preserved frame_index: {sample['frame_index']}")
        if "index" in data:
            sample["index"] = data["index"]
            logger.debug(f"[FastWAMProcessor.preprocess] Preserved index: {sample['index']}")
        if "dataset_index" in data:
            sample["dataset_index"] = data["dataset_index"]
            logger.debug(f"[FastWAMProcessor.preprocess] Preserved dataset_index: {sample['dataset_index']}")

        # sample = self.tokenizer(sample)
        
        logger.debug(f"[FastWAMProcessor.preprocess] Final sample keys: {list(sample.keys())}")
        return sample
