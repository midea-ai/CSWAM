...
class LeRobotDatasetMetadata:
    def __init__(
        ...
        try:
            ...
            logging.info("Dataset metadata not found under %s; downloading it.", meta_info_path.parent)
    ...
    def pull_from_repo(
        ...
        logging.info("Downloading dataset metadata for %s into %s.", self.repo_id, self.root)
        snapshot_download(
            self.repo_id,
            repo_type="dataset",
    ...
    def get_data_file_path(self, ep_index: int) -> Path:
        ...
        if self._version >= packaging.version.parse("v3.0") and ep_index in self.episodes:
            ...
            chunk_index = ep_dict.get("data_chunk_index", 0)
            file_index = ep_dict.get("data_file_index", 0)
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        try:
            ...
            if missing:
                logging.info(
                    "Dataset %s is missing %d local files; downloading them.",
                    self.repo_id,
                    len(missing),
                )
        ...
        # Convert HF Dataset columns to tensors (handle both v2.1 list and v3.0 Column)
        if isinstance(timestamps_col, list):
            timestamps = torch.stack(timestamps_col).numpy()
            episode_indices = torch.stack(episode_indices_col).numpy()
        else:
            # For HF Dataset Column, convert to tensor first
            timestamps = torch.tensor(timestamps_col, dtype=torch.float32).numpy()
            episode_indices = torch.tensor(episode_indices_col, dtype=torch.int64).numpy()
    ...
    def _query_videos(self, query_timestamps: dict[str, list[float]], ep_idx: int) -> dict[str, torch.Tensor]:
        ...
        for vid_key, query_ts in query_timestamps.items():
            ...
            # v3.0: multiple episodes share one video file, need timestamp offset
            if self.meta._version >= packaging.version.parse("v3.0") and ep_idx in self.meta.episodes:
                ep_dict = self.meta.episodes[ep_idx]
                ts_offset = ep_dict.get(f"videos/{vid_key}/from_timestamp", 0.0)
                if ts_offset:
                    query_ts = [ts + ts_offset for ts in query_ts]
    ...
    def __getitem__(self, idx) -> dict:
        ...
        if self.delta_indices is not None:
            # O(1) 查找：用预建字典替代 list.index()（O(N)），对大数据集每帧节省数毫秒
    ...
    def get_episode_data(self, episode_idx: int, columns: list[str] | None = None) -> dict:
        for dataset in self._datasets:
            if episode_idx < dataset.num_episodes:
                ...
                table = pq.read_table(str(file), columns=columns)
