...
class FastWAM(torch.nn.Module):
    ...
    def _append_proprio_to_context(
        ...
        #Tim's 0pro setting
        # print("!!!!!!!!!!attention proprio is set to 0 now!!!!!!!")
    ...
    def build_inputs(self, sample, tiled: bool = False):
        ...
        if action_dim_is_pad is not None:
            if action_dim_is_pad.ndim == 1 and batch_size == 1:
                action_dim_is_pad = action_dim_is_pad.unsqueeze(0)
            if action_dim_is_pad.ndim != 2:
                raise ValueError(
                    "`sample['action_dim_is_pad']` must be 2D [B, D], "
                    f"got shape {tuple(action_dim_is_pad.shape)}"
                )
            expected_shape = (batch_size, int(action.shape[2]))
            if tuple(action_dim_is_pad.shape) != expected_shape:
                raise ValueError(
                    "`sample['action_dim_is_pad']` shape mismatch: "
                    f"got {tuple(action_dim_is_pad.shape)} vs expected {expected_shape}"
                )
        ...
        self._training_timing_mark("input_h2d")
        input_latents = self._encode_video_latents(input_video, tiled=tiled)
        self._training_timing_mark("vae_encode")
    ...
    def _training_timing_mark(self, name: str) -> None:
        """Optional training profiler hook overridden by instrumented models."""
        del name
    ...
    def _compute_action_loss_per_sample(
        ...
        """Average action MSE over valid timesteps and valid dimensions."""
        element_loss = F.mse_loss(
            pred_action.float(), target_action.float(), reduction="none"
        )
        valid = torch.ones_like(element_loss)
        if action_is_pad is not None:
            valid = valid * (~action_is_pad).unsqueeze(-1).to(valid.dtype)
        if action_dim_is_pad is not None:
            valid = valid * (~action_dim_is_pad).unsqueeze(1).to(valid.dtype)
        valid_sum = valid.sum(dim=(1, 2)).clamp(min=1.0)
        return (element_loss * valid).sum(dim=(1, 2)) / valid_sum
