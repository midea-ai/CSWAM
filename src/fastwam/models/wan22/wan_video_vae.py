...
class WanVideoVAE(nn.Module):
    ...
    def single_encode(self, video, device):
        ...
        x = self._run_model_encode(video)
        return x


    def configure_encode_optimization(
        self,
        *,
        compile_encoder=False,
        compile_mode="default",
    ):
        """Configure the inference-only VAE encoder without changing state keys."""
        self.compile_encoder = bool(compile_encoder)
        self.compile_mode = str(compile_mode)
        object.__setattr__(self, "_compiled_model_encode", None)
        if not self.compile_encoder:
            return
        if not hasattr(torch, "compile"):
            raise RuntimeError("VAE compile requires a PyTorch build with torch.compile().")
        ...
        # A compiled bound method is deliberately not registered as a child
        # module, preserving all existing VAE and FastWAM checkpoint keys.
        object.__setattr__(self, "_compiled_model_encode", compiled_encode)


    def _run_model_encode(self, video):
        encode = getattr(self, "_compiled_model_encode", None)
        if encode is None:
            encode = self.model.encode
        return encode(video, self.scale)
    ...
    def encode_batched(
        ...
        *,
        ...
        """Encode [B,C,T,H,W] clips in fixed-size VAE micro-batches."""
        if not isinstance(videos, torch.Tensor) or videos.ndim != 5:
            shape = tuple(videos.shape) if isinstance(videos, torch.Tensor) else type(videos)
            raise ValueError(f"`videos` must be a 5D tensor [B,C,T,H,W], got {shape}")
        if tiled:
            return self.encode(
                videos,
                device=device,
        ...
        hidden_states = []
        for start in range(0, videos.shape[0], micro_batch_size):
            video_batch = videos[start:start + micro_batch_size].to(device)
            hidden_states.append(self._run_model_encode(video_batch))
