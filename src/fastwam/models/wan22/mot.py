...
class MoT(nn.Module):
    def __init__(
        ...
        if "action" not in mixtures:
            raise ValueError("`mixtures` must include an 'action' expert.")
        if len(mixtures) < 2:
            raise ValueError("`mixtures` must include at least one context expert and the 'action' expert.")
    ...
    def prefill_static_expert_cache(
        self,
        expert_inputs: Dict[str, dict],
        attention_mask: torch.Tensor,
        *,
        static_expert_order: Optional[list[str]] = None,
    ) -> dict:
        """Run non-action experts jointly once and cache their layer-wise K/V.

        This is the generalized cache path used by CSWAM inference.  The
        static experts can attend to one another during the prefill, while the
        action expert is omitted and remains step-dependent during denoising.
        The returned cache is intentionally a small plain dictionary so it can
        be reused by the existing action control loop.
        """
        if static_expert_order is None:
            static_expert_order = [name for name in self.expert_order if name != "action"]
        static_expert_order = list(static_expert_order)
        if not static_expert_order:
            raise ValueError("At least one static expert is required for cache prefill.")
        missing = [name for name in static_expert_order if name not in self.mixtures]
        if missing:
            raise ValueError(f"Unknown static experts: {missing}")
        missing = [name for name in static_expert_order if name not in expert_inputs]
        if missing:
            raise ValueError(f"Missing static expert inputs: {missing}")
        if attention_mask.ndim != 2 or attention_mask.shape[0] != attention_mask.shape[1]:
            raise ValueError("Static attention mask must be square [S,S].")

        tokens_all = {name: expert_inputs[name]["tokens"] for name in static_expert_order}
        seq_lens = [int(tokens_all[name].shape[1]) for name in static_expert_order]
        expected_seq_len = sum(seq_lens)
        if attention_mask.shape[0] != expected_seq_len:
            raise ValueError(
                f"Static attention mask length={attention_mask.shape[0]} "
                f"does not match static token length={expected_seq_len}."
            )

        kv_cache = {name: [] for name in static_expert_order}
        for layer_idx in range(self.num_layers):
            q_chunks, k_chunks, v_chunks = [], [], []
            cached = {}
            for name in static_expert_order:
                expert = self.mixtures[name]
                block = expert.blocks[layer_idx]
                state = expert_inputs[name]
            ...
            start = 0
            for name, seq_len in zip(static_expert_order, seq_lens):
                end = start + seq_len
                state = cached[name]
                ...
                kv_cache[name].append({"k": state["k"], "v": state["v"]})
                start = end

        return {
            "expert_order": static_expert_order,
            "seq_lens": seq_lens,
            "total_seq_len": expected_seq_len,
            "layers": kv_cache,
        }
    ...
    def forward_action_with_static_cache(
        ...
        """Run the action expert against cached video/JEPA K/V features."""
        if "action" not in self.mixtures:
            raise ValueError("MoT requires an `action` expert for action decoding.")
        static_names = list(static_cache.get("expert_order", []))
        static_seq_len = int(static_cache.get("total_seq_len", 0))
        if not static_names or static_seq_len <= 0:
            raise ValueError("Invalid static cache: no cached expert sequence.")
        if len(static_cache.get("layers", {})) != len(static_names):
            raise ValueError("Static cache layer payload is malformed.")
        action_tokens = action_inputs["tokens"]
        action_seq_len = int(action_tokens.shape[1])
        expected_total = static_seq_len + action_seq_len
        if attention_mask.shape != (expected_total, expected_total):
            raise ValueError(
                f"Joint cache attention mask must be {(expected_total, expected_total)}, "
                f"got {tuple(attention_mask.shape)}"
            )
        action_attention_mask = attention_mask[static_seq_len:, :]
        expert = self.mixtures["action"]
        x = action_tokens
        layers = static_cache["layers"]
        for layer_idx in range(self.num_layers):
            block = expert.blocks[layer_idx]
            q, k_action, v_action, residual_x, gate_msa, shift_mlp, scale_mlp, gate_mlp, use_ckpt = (
                self._build_expert_attention_io(
                    expert=expert,
                    block=block,
                    x=x,
                    freqs=action_inputs["freqs"],
                    t_mod=action_inputs["t_mod"],
                )
            )
            k_static = [layers[name][layer_idx]["k"] for name in static_names]
            v_static = [layers[name][layer_idx]["v"] for name in static_names]
            mixed = self._mixed_attention(
                q_cat=q,
            ...
            x = self._apply_post_with_optional_checkpoint(
                ...
                mixed_slice=mixed,
                context_payload={
                    "context": action_inputs.get("context"),
                    "mask": action_inputs.get("context_mask"),
                },
            )
        return x
