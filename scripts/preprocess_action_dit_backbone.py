...
def _load_model_config(path: Path) -> tuple[dict[str, Any], dict[str, Any], Any]:
    ...
    if "video_dit_config" not in model_cfg or "action_dit_config" not in model_cfg:
        raise ValueError(
            f"`{path}` must contain `video_dit_config` and `action_dit_config` "
            "either at the top level or under `model`."
        )

    video_cfg = OmegaConf.to_container(model_cfg.video_dit_config, resolve=True)
    action_cfg = OmegaConf.to_container(model_cfg.action_dit_config, resolve=True)
    ...
    return video_cfg, action_cfg, model_cfg
...
def main() -> None:
    ...
    parser.add_argument(
        "--model-config",
        required=True,
        help="Path to a CSWAM training YAML or a standalone model YAML.",
    )
